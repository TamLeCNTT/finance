            ...
            if (!soluong)
                ...
                if (!content)
                    ...
                    if (!state)
                        ...
                        let paid = { id: makeid(10), content: content, date: date, quantity: soluong, status: Number(state) + 1 }
                        console.log(paid)
                        ThuchiService.getAll().then(res => {
                            let listpaid = res.data ? res.data.filter(item => item !== null)[state] ? res.data.filter(item => item !== null)[state].data : [] : []

                            listpaid.push(paid)
                            console.log(listpaid)
                            ThuchiService.update({ name: Number(state) + 1, data: listpaid }).then(
                                ress => {
                                    toast.success("ok")
                                }
                            )
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title id="example-custom-modal-styling-title">
                        <h1 id="title" className="text-center">Add finance</h1>
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
