...
const ThuChiList = () => {
    ...
    useEffect(() => {
        ThuchiService.getAll().then(res => {
            if (res.data) {
                let listpaid = res.data.filter(item => item !== null)[2] ? res.data.filter(item => item !== null)[2].data : []
                let datebypaid = listpaid ? listpaid[listpaid.length - 1].date : []
                let listretract = res.data.filter(item => item !== null)[0] ? res.data.filter(item => item !== null)[0].data.filter(item => new Date(item.date) >= new Date(datebypaid)) : []
                let listSpend = res.data.filter(item => item !== null)[1] ? res.data.filter(item => item !== null)[1].data.filter(item => new Date(item.date) >= new Date(datebypaid)) : []

                setRetract(listretract.reduce((a, v) => a = a + Number(v.quantity), 0))
                ...
                setListShow(listShow.sort((a, b) => new Date(b.date) - new Date(a.date)))
            }
            ...
            <main id="cabin_list" className="main">
                <div className="container">
                    ...
                    <div className="row bg-success p-3 text-white">
                        <div className="col col-md-4 d-flex   justify-content-center">{retract ? retract : 0}</div>
                        <div className="col col-md-4 d-flex   justify-content-center">{spend ? spend : 0}</div>
                        <div className="col col-md-4 d-flex  justify-content-center">{retract - spend ? retract - spend : 0}</div>
                                    ...
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                ...
                                                <input type='button' className='btn btn-danger' value={"edit"} />
                                            </td>
